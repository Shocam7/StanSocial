...
export function VideoTile({ post, layout, position }: VideoTileProps) {
  ...
  // Determine video height based on layout
  ...
  return (
    <Card className="overflow-hidden hover:shadow-lg transition-shadow group cursor-pointer h-full">
      <div className="relative">
        ...
        <div className={`relative w-full ${getVideoHeight()} bg-black`}>
          <img src={post.video || "/placeholder.svg"} alt="Video thumbnail" className="w-full h-full object-cover" />

          {/* Play button overlay */}
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="bg-black/50 rounded-full p-2 md:p-4 group-hover:bg-black/70 transition-colors">
              <Play className={`${layout === "third" ? "h-6 w-6" : "h-8 w-8"} text-white fill-white`} />
            </div>
          </div>
